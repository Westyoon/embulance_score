...
export default function AnalyticsTab({ data }) {
  ...
  const expiredCount = analysisSnapshot?.expiredRegions
    ...
    ?? 0;
  const isHistoricalSnapshot = expiredCount > 0;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div role="alert" style={{ color: "#92400e", background: "#fffbeb", border: "1px solid #fde68a", borderRadius: 12, padding: "12px 14px", fontSize: 11.5, lineHeight: 1.6 }}>
          ...
          {isHistoricalSnapshot ? (
            <>
              ...
              기준시각 내 최신 지역은 <b>{currentComplete}개</b>이며, 병상 원천 기준시각이 지난 <b>{expiredCount}개</b>도 값이 사라지지 않도록 마지막 계산값을 포함합니다.
            ...
            <>
              평균·순위·트리맵·버블차트는 산출 점수가 있는 <b>{sourceComplete}개 지역</b>을 포함합니다.
              원천 결측 <b>{sourceMissingCount}개</b>는 0점으로 대체하지 않고 집계와 차트에서 제외했습니다.
            </>
          )}
          {(expiredRegionRows.length > 0 || sourceMissingRows.length > 0 || sourcePolicyInvalidRows.length > 0) && (
            <details style={{ marginTop: 6 }}>
              <summary style={{ cursor: "pointer", fontWeight: 600 }}>기준시각 경과·결측·원천기준 주의 지역 보기</summary>
              <div style={{ marginTop: 6, color: "#78350f" }}>
                {expiredRegionRows.length > 0 && (
                  <div><b>마지막 계산값:</b> {expiredRegionRows.map((region) => `${region.sido ?? ""} ${region.name}`.trim()).join(" · ")}</div>
                )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        ...
        <div style={{ ...cardStyle, padding: 16 }}>
          ...
          <div style={{ fontSize: 10.5, ...mutedText, marginBottom: 8 }}>x=응급실 수 · y=의료진 수 · 크기=인구대비병상 부담 · 색상=위험도 · {isHistoricalSnapshot ? `마지막 계산 ${bubbleData.length}개 지역` : `산출 ${bubbleData.length}개 지역 표시`}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        {ranked.length > 0 ? (
          ...
          <AnalysisUnavailableCard
            ...
            message="산출된 위험도 점수가 없어 평균 기여 점수를 계산하지 않았습니다. 원천 결측 지역을 0점으로 대체하지 않습니다."
          />
        )}
