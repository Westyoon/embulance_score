...
| PART 3 의료진 부족 | 부분 완료 | HIRA 531개 매칭(자동 523개·수동검증 8개), 218개 지역 사용 가능 |
| PART 3 최종 regionRisk | 부분 완료 | 219개 중 197개 산출 완료, 22개 원천데이터부족 |
| PART 4 상관관계·VIF | 완료 | 최종 점수가 완성된 지역 대상 |
| PART 4 선형회귀 | 완료 | 완료지역 197개 전체, R² -1.168, MAE 5.22 |
| PART 4 K-Means | 완료 | 197개 지역, 최적 k=2 |
| 최신 시군구 경계 | 완료 | `admdongkor` `20260701`, 256개 경계 |
| PART 4 과거 시간대 분석 | 미완료 | 2024년 병상 이력 원자료 필요 |

최종 위험도가 없는 22개 지역은 임의 점수로 대체하지 않고 `원천데이터부족`으로 표시합니다.
...
- HIRA 전체 매칭: 531개
  ...
  - 수동검증: 8개
- 검색 결과 없음: 0개
- 동일 건물의 재활병원 후보 2개와 HIRA 원천 불일치 1개 보류
- 지역별 80% 매칭 기준: 218개 지역 통과, 서울 관악구 75%로 검토 필요

동일 HIRA 식별자로 잘못 연결됐던 명지병원 2건과 기존 지역 불일치·식별자 충돌 6건은 공식 HIRA 기관 페이지를 근거로 분리 검증했습니다. 강남힐병원(`A1100076`)은 NEMC·서울시 응급의료기관 목록에는 있으나 2026-08-31 HIRA 전체목록과 전문의 API에 없고 HIRA 상세 URL도 “병원정보가 존재하지 않습니다”를 반환합니다. 전문의 1명을 임의 유지하지 않고 HIRA 보강값을 결측으로 두었으며, 이 때문에 관악구는 4개 중 3개만 연결되어 80% 기준을 통과하지 못합니다.

- `A2100042` 고양 명지병원: 응급의학과 전문의 12명
- `A2300009` 제천 명지병원: 응급의학과 전문의 7명

현재 보류된 3개 병원과 향후 새 미매칭 기관은 다음 절차로 처리합니다.
...
- `data/hira_match_overrides.csv`: 근거 URL과 확인일을 포함한 수동검증 8개
...
`.env`는 Git에서 제외됩니다. 실제 값은 로컬 `.env` 또는 배포 플랫폼의 비밀변수에만 넣습니다. `PIPELINE_ADMIN_TOKEN`은 동적 서버의 수동 갱신 API에만 필요하며 원천 API 키와 마찬가지로 `NEXT_PUBLIC_` 접두사를 사용하면 안 됩니다. 스크립트가 파일을 직접 읽으므로 VS Code의 `python.terminal.useEnvFile` 설정은 필요하지 않습니다. 운영 기본값을 포함한 전체 예시는 `.env.example`을 참고합니다.
...
전체 HIRA 목록을 시군구별 후보 풀로 사용하고 이름·주소·전화·좌표·시설종별을 함께 비교합니다. 후보는 요양기호가 중복되지 않도록 전역 1:1로 배정하며, 공식 페이지 수동검증은 `hira_match_overrides.csv`에 근거 URL과 확인일을 보존합니다. 고양·제천 명지병원을 포함한 8개 수동검증과 523개 자동매칭으로 218개 지역이 80% 기준을 통과하며, HIRA 원천에서 사라진 강남힐병원 때문에 관악구 1곳은 검토 대상으로 남습니다.
