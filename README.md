...
기본 병상 수집 주기는 `Asia/Seoul` 기준 21:00~09:00에 2분, 09:00~21:00에 10분입니다. 성공 완료 후 다음 실행까지의 목표 간격이므로 수집 작업이나 일일 전체 갱신이 오래 걸리면 실효 간격은 길어질 수 있습니다. 프로세스를 계속 실행해야 주기가 유지되며 종료는 `Ctrl+C`입니다. `PIPELINE_ADMIN_TOKEN`은 예약 수집에는 필요하지 않고 로컬 수동 갱신 API를 함께 사용할 때만 설정합니다. 실제 키는 출력하거나 Git에 커밋하지 않습니다.
...
- 시간대별 병상 주기: 한국시간 21:00~09:00는 2분, 그 밖에는 10분
