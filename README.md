...
> 기관 수, 산출 지역 수, 결측 수와 수집시각은 배치마다 달라집니다. README의 숫자를 운영 현황으로 간주하지 않고, 항상 [`GET /api/health`](https://emergency-dashboard-production-e303.up.railway.app/api/health)의 `dataAsOf`, `scoredRegions`, `completeRegions`, `bedRiskStaleRegions`, `pipeline`을 확인합니다.
...
- 오래된 병상은 원래 `API기준시각`·`수집시각`과 함께 마지막 정상값으로 보존하고, API 무응답·이상값과 낮은 HIRA 매칭률은 0으로 대체하지 않고 품질 상태로 추적합니다.
- 수집·분석·프론트 데이터 계약을 모두 통과한 staging만 영속 Volume에 승격합니다.
- 병상 원천 기준시각이 지나도 지도·상세·분석에서 마지막 수집값과 계산 점수를 유지하되, 실시간 값으로 오해하지 않도록 기준시각 경과 경고와 원래 시각을 함께 표시합니다.
...
- 전체병상 누락·0, 음수 가용병상처럼 수치가 유효하지 않은 행은 병상 결측으로 처리합니다. API 무응답 시에는 마지막으로 수치가 유효했던 행을 원래 시각 그대로 유지합니다.
...
- 병상 원천 기준시각이 지난 값과 점수도 숨기지 않으며 `마지막 수집값`·`이전값`으로 표시합니다. `API기준시각`과 `수집시각`은 갱신 성공 전까지 바꾸지 않고, 신선도는 경고 메타데이터로 제공합니다.
...
같은 컴퓨터에서 대시보드와 정기 수집기를 계속 실행하려면 운영 빌드를 만든 뒤 로컬 파이프라인 명령을 사용합니다. 이 명령은 `.env`의 세 서버 API 키를 먼저 확인하고 스케줄러를 활성화하며, 저장소의 `data/`가 아닌 Git 제외 경로인 `runtime/data/`에 검증을 통과한 세대만 승격합니다.

```powershell
Copy-Item .env.example .env  # 최초 한 번; 세 API 키를 .env에 입력
npm run build
npm run check:local-pipeline
npm run start:local-pipeline
```

기본 병상 수집 주기는 `Asia/Seoul` 기준 8시간입니다. 21:00 핵심 시간대 경계에 맞춘 뒤 정상 성공 기준으로 21:00·05:00·13:00, 하루 3회 실행합니다. 현재 219개 지역에서는 하루 657회 API 호출을 사용해 1,000회 할당량 가운데 343회를 실패 재시도와 수동 점검 여유로 남깁니다. 성공 완료 후 다음 실행까지의 목표 간격이므로 수집 작업이나 일일 전체 갱신이 오래 걸리면 실효 간격은 길어질 수 있습니다. 프로세스를 계속 실행해야 주기가 유지되며 종료는 `Ctrl+C`입니다. `PIPELINE_ADMIN_TOKEN`은 예약 수집에는 필요하지 않고 로컬 수동 갱신 API를 함께 사용할 때만 설정합니다. 실제 키는 출력하거나 Git에 커밋하지 않습니다.
...
테스트는 모집단·HIRA 1:1 매칭·결측 리포트·경계 연결·위험등급·마지막 정상값 보존과 신선도 경고·지도 hover·시간대별 병상 우선 스케줄 정책을 포함합니다. CI는 같은 검증을 수행한 뒤 Docker 이미지를 만들고 실제 `/api/health`, HTML, 정적 asset과 Volume 동작을 smoke test합니다.
...
`npm run start:dynamic`은 Next.js 서버와 Python 스케줄러를 한 컨테이너에서 실행합니다. 로컬에서는 키·빌드·쓰기 경로를 먼저 검사하고 스케줄러를 강제로 켜는 `npm run start:local-pipeline`을 사용합니다.

- `beds`: NEMC 병상과 이에 의존하는 점수·분석을 갱신
- `full`: 기관·인구·HIRA·경계·카카오와 전체 분석을 갱신
- 병상 주기: 한국시간 21:00 핵심 시간대 경계에 맞춘 8시간 간격(정상 성공 기준 21:00·05:00·13:00, 하루 3회)
- NEMC 기관 일시 누락: 신규 코드 없는 최대 3곳만 이전 검증 행을 3회·72시간 한도로 승계하고 감사 기록
- 소수 지역 5xx: 성공 지역은 반영하고 실패 지역은 수치가 정상인 마지막 값을 원래 시각 그대로 보존한 뒤 45분 이내 재시도
- 동시 지연: 원천 신선도 경계가 가까운 `beds`를 우선 실행하고, 기한이 된 일일 `full`은 성공한 `beds` 직후 실행하며 그동안 도래한 병상 작업은 하나로 합쳐 종료 직후 실행
