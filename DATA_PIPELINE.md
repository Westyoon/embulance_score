...
`admdongkor` 경계는 SGIS 원천을 가공한 웹 시각화용 단순화 자료이며 법적·측량·주소 판정용 공식 원본이 아닙니다. 경계 데이터의 CC BY 4.0·공공누리 제1유형과 생성 코드의 MIT 조건은 서로 구분하며, 버전·출처·이용조건은 `src/data/KOREA_GEO_LICENSE.md`에 보존합니다.
