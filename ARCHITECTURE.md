...
`scripts/start_dynamic.mjs`는 Next.js 서버와 Python 자식 프로세스를 함께 관리합니다. 스케줄러는 한 번에 하나의 갱신만 실행하고, 중복 요청은 단일 대기 작업으로 합칩니다. 병상 기본 주기는 `Asia/Seoul` 기준 핵심 시간대 21:00~09:00에는 2분, 그 밖에는 10분이며, 최근 성공시각뿐 아니라 병원 원천 기준시각의 가장 이른 신선도 경계도 추적합니다. `beds`와 `full`이 함께 밀리면 병상 갱신을 먼저 실행합니다. 짧은 병상 주기 때문에 일반적인 full 실행 여유가 생기지 않으므로, 이미 기한이 된 일일 `full`은 선행 deadline·retry가 없을 때 성공한 `beds` 직후 1분 이내에 시작합니다. 실행 중 도래한 병상 작업은 하나로 합쳐 `full` 종료 직후 실행합니다. Railway에서 스케줄러를 켤 때는 영속 Volume과 단일 replica가 필수입니다. 로컬의 `npm run start:local-pipeline`은 키·빌드·쓰기 경로를 확인한 뒤 같은 런타임을 `runtime/`에 연결합니다.
...
- 병상 갱신은 현재 NEMC 분석 지역별 API 요청이 필요합니다. 핵심 시간대 2분·비핵심 시간대 10분 설정에는 하루 약 432회분의 지역별 호출과 실패 재시도 여유가 필요하므로, 현재 모집단 기준으로 공공데이터포털 할당량을 확인해야 합니다.
