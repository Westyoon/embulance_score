...
| `GET /api/health` | 데이터 신선도, 기준시각 경과 병상, 파이프라인 상태 제공 |
| `POST /api/ops/refresh` | Bearer 토큰으로 `beds` 또는 `full` 갱신 요청 접수 |

`scripts/start_dynamic.mjs`는 Next.js 서버와 Python 자식 프로세스를 함께 관리합니다. 스케줄러는 한 번에 하나의 갱신만 실행하고, 중복 요청은 단일 대기 작업으로 합칩니다. 병상 기본 주기는 `Asia/Seoul` 기준 8시간이며 21:00 핵심 시간대 경계에 맞춘 뒤 정상 성공 기준 21:00·05:00·13:00에 실행합니다. 최근 성공시각뿐 아니라 병원 원천 기준시각의 가장 이른 신선도 경계도 추적합니다. `beds`와 `full`이 함께 밀리면 병상 갱신을 먼저 실행하고, 실행 중 도래한 병상 작업은 하나로 합쳐 `full` 종료 직후 실행합니다. 운영자가 병상 간격을 `full` 실행 보호시간보다 짧게 재설정한 경우에는 이미 기한이 된 일일 `full`을 선행 deadline·retry가 없을 때 성공한 `beds` 직후 1분 이내에 시작합니다. Railway에서 스케줄러를 켤 때는 영속 Volume과 단일 replica가 필수입니다. 로컬의 `npm run start:local-pipeline`은 키·빌드·쓰기 경로를 확인한 뒤 같은 런타임을 `runtime/`에 연결합니다.
...
네 항목이 모두 존재할 때만 최종 점수를 계산합니다. 모집단·완료·최근 계산·기준시각 경과 지역 수는 배치마다 달라지므로 `/api/health`의 `regions`, `completeRegions`, `scoredRegions`, `bedRiskStaleRegions`를 기준으로 확인합니다. `expiredScoreRegions`는 호환 필드명이며 해당 점수도 마지막 계산값으로 표시됩니다. 이 값은 의료적 진단이나 이송 지시가 아니라 데이터 기반 지역 비교 지표입니다.

신뢰성 방어선은 다음과 같습니다.

- NEMC 기관 수, 지역 수, HIRA 매칭 수, 수치가 정상인 병상 기관 수가 검토 기준보다 급감하면 승격을 중단합니다.
- 전체병상 0·누락, 음수 가용병상, 계산식과 맞지 않는 포화율은 유효값으로 사용하지 않습니다.
- 병원이 보고한 `API기준시각`이 운영 기준 12시간보다 오래돼도 수치가 정상이면 원래 시각과 함께 마지막 정상값으로 보존하고 `last-known` 신선도 상태를 붙입니다.
...
서버 스냅샷은 수치와 신선도 상태를 분리합니다. 병상 기준시각이 지난 병원·지역도 메인 지도, 상세와 분석에서 마지막 정상 병상값·위험도를 유지하되 `bedDataStale`/`bedRiskStale`, `last-known` 상태와 원래 기준시각을 함께 제공합니다. `analysisSnapshot`에는 마지막 계산시각, 현재 기준시각 경과 여부와 계산 당시 원천정책 충족 여부를 보존합니다. 따라서 “값이 존재함”과 “현재 시점의 최신값임”을 같은 상태로 취급하지 않습니다.
...
- 병상 갱신은 현재 NEMC 분석 지역별 API 요청이 필요합니다. 기본 8시간 간격은 하루 3회분의 지역별 호출을 사용합니다. 현재 219개 지역에서는 657회이고, 1,000회 할당량의 나머지는 실패 재시도와 수동 점검 여유로 둡니다.
